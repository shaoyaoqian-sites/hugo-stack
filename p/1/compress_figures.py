...
def reduce_image_quality(infile, mb=10240000, step=3, quality=90):
    """不改变图片尺寸压缩到指定大小
    :param infile: 压缩源文件
...
for file_input in path_list:
    ...
    out.save(file_output, quality=80)
...
for file_path in existed_path_list:
    file_name = os.path.basename(file_path)
    compress_file_list.append(file_name)
